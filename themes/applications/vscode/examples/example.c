// See: https://learnxinyminutes.com/docs/c/
...
int main(int argc, char **argv) {
  ...
  // argument is not evaluated (except VLAs (see below)). The value it yields in
  // this case is a compile-time constant.
  int a = 1;
  ...
  // is equivalent:
  char my_array[] = {0};
  // BUT, then you have to evaluate the size of the array at run-time, like
  // this:
  ...
  // You need to cast at least one integer to float to get a floating-point
  // result
  (float)i1 / i2;  // => 0.5f
...
error: // this is a label that you can "jump" to with "goto error;"
  ...
  */
  ...
  // pointer, it decays into (implicitly converted to) a pointer. Exceptions:
  ...
  // literal, since string literals shall not be modified (i.e. "foo"[0] = 'a'
  // is ILLEGAL.)
...
void str_reverse(char *str_in) {
  ...
  size_t ii = 0;
  size_t len =
      strlen(str_in); // `strlen()` is part of the c standard library
                      // NOTE: length returned by `strlen` DOESN'T include the
                      //       terminating NULL byte ('\0')
  for (ii = 0; ii < len / 2;
       ii++) { // in C99 you can directly declare type of `ii` here
    tmp = str_in[ii];
...
// the whole struct:
int areaptr(const rect *r) { return r->width * r->height; }
