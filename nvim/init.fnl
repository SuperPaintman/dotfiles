...
(local { :expand expand
         ...
         :filereadable file-readable? } vim.fn)
...
(macro command [name ...]
  ...
  (fn create-consumer [name ?checker]
    (fn [tbl n]
      (let [(v move) (if (not= ?checker nil) (?checker (. tbl n)) (values true false))]
        (tset opts name v)
        (consume tbl (+ n (if move 1 0))))))

  ;; See: https://neovim.io/doc/user/map.html#command-attributes
  (local consume-range (create-consumer :range #(match $1
    :% (values :% true)
    _ (values true false))))
